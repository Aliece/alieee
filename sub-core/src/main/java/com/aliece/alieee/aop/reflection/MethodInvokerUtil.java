...
public class MethodInvokerUtil {
	...
	public Object execute(Method method, Object targetObj, Object[] p_args) throws Throwable {
		try {
			if ((method == null) || (targetObj == null))
				Debug.logError("[alieee] no method or target, please check your configure", module);
			if (p_args == null)
				p_args = new Object[0];
			Debug.logVerbose("[alieee] method invoke: " + targetObj.getClass().getName() + " method=" + method.getName(), module);
			Object result = method.invoke(targetObj, p_args);
			Debug.logVerbose("[alieee] method invoke successfully ", module);
			return result;
			...
			errorInfo = "[alieee] method invoke IllegalArgumentException: " + iex + " method argument type :[" + method.getParameterTypes()
			...
			String errorInfo = "Errors happened in your method:[" + targetObj.getClass().getName() + "." + method.getName() + "]";
			Debug.logError(errorInfo, module);
			Debug.logError("[alieee] method invoke IllegalAccessException: " + ex, module);
			throw new Throwable("access method:" + method + " " + ex, ex);
		} catch (Exception ex) {
			String errorInfo = "Errors happened in your method:[" + targetObj.getClass().getName() + "." + method.getName() + "]";
			Debug.logError(errorInfo, module);
			Debug.logError("[alieee] method invoke error: " + ex, module);
	...
	public Object createTargetObject(TargetServiceFactory targetServiceFactory) {
		Debug.logVerbose("[alieee] now getTargetObject by visitor ", module);
		...
		try {
			...
			if (targetMetaDef.isEJB()) { // cache the ejb object
				...
				Debug.logVerbose("[alieee] not active targer service instance cache !!!!", module);
				targetObjRef = targetServiceFactory.create();
			}
		} catch (Exception e) {
			Debug.logError("[alieee]createTargetObject error: " + e, module);
		}
		return targetObjRef;
	}
